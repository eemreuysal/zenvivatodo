...
class DatabaseHelper {
  ...
  Future<void> _onCreate(Database db, int version) async {
    ...
    await db.insert('categories', const models.Category(name: 'İş', color: 0xFF1565C0).toMap());
    await db.insert(
      'categories',
      const models.Category(name: 'Kişisel Gelişim', color: 0xFF673AB7).toMap(),
    );
    await db.insert('categories', const models.Category(name: 'Sağlık', color: 0xFF4CAF50).toMap());
  }
  ...
  Future<void> _onUpgrade(Database db, int oldVersion, int newVersion) async {
    ...
    if (oldVersion < 4) {
      ...
      await _executeAlterTableSafely(
        db,
        'ALTER TABLE users ADD COLUMN created_at TEXT DEFAULT CURRENT_TIMESTAMP',
      );
      await _executeAlterTableSafely(db, 'ALTER TABLE users ADD COLUMN last_login TEXT');

      await _executeAlterTableSafely(db, 'ALTER TABLE tasks ADD COLUMN uniqueId TEXT');
      await _executeAlterTableSafely(
        db,
      ...
      await _createIndexIfNotExists(
        db,
        'idx_task_tag_relations_taskId',
        'task_tag_relations',
        'taskId',
      );
      await _createIndexIfNotExists(
        db,
        'idx_task_tag_relations_tagId',
        'task_tag_relations',
        'tagId',
      );
    }
  ...
  Future<List<Task>> searchTasks(int userId, String query, {bool includeCompleted = false}) async {
    ...
    final List<Map<String, dynamic>> maps = await db.query(
      ...
      whereArgs: [userId, '%$query%', '%$query%', isCompletedValue],
      orderBy: 'date ASC, time ASC',
    );
  ...
  Future<int> toggleTaskCompletion(int id, bool isCompleted) async {
    ...
    return await db.update(
      'tasks',
      {'isCompleted': completedValue, 'updated_at': DateTime.now().toIso8601String()},
      where: 'id = ?',
      whereArgs: [id],
    );
  }
  ...
  Future<List<Map<String, dynamic>>> getTags(int userId) async {
    final Database db = await database;
    return await db.query('task_tags', where: 'userId = ?', whereArgs: [userId]);
  }
  ...
  Future<void> assignTagToTask(int taskId, int tagId) async {
    final Database db = await database;
    await db.insert('task_tag_relations', {'taskId': taskId, 'tagId': tagId});
  }
    ...
    try {
      await db.transaction((txn) async {
        for (var task in tasks) {
          ...
          await txn.update('tasks', taskMap, where: 'id = ?', whereArgs: [task.id]);
        }
      });
      return true;
    } on DatabaseException catch (e) {
  ...
  Future<List<Map<String, dynamic>>> getCompletedTasksLast7Days(int userId) async {
    ...
    for (final date in dates) {
      ...
      if (result.isNotEmpty) {
        ...
        mappedResults.add({'date': date, 'count': 0});
      }
    }
  ...
  Future<int> updateHabit(Map<String, dynamic> habit) async {
    ...
    return await db.update('habits', habit, where: 'id = ?', whereArgs: [habit['id']]);
  }
  ...
  Future<int> archiveHabit(int id, bool isArchived) async {
    ...
    return await db.update(
      'habits',
      {'isArchived': archivedValue, 'updated_at': DateTime.now().toIso8601String()},
      where: 'id = ?',
      whereArgs: [id],
    );
  }
  ...
  Future<int> toggleShowInDashboard(int id, bool showInDashboard) async {
    ...
    return await db.update(
      'habits',
      {'showInDashboard': dashboardValue, 'updated_at': DateTime.now().toIso8601String()},
      where: 'id = ?',
      whereArgs: [id],
    );
  }
  ...
  Future<List<Map<String, dynamic>>> getHabitLogs(int habitId, {String? date}) async {
    ...
    // Koşullu ifade yerine if koşulu kullanıldı
    if (date != null) {
      return await db.query(
        'habit_logs',
