...
class ApiService {
  ...
  Future<List<Task>> getTasksByDate(int userId, String date) async {
    try {
      final response = await _dio.get('/tasks', queryParameters: {'userId': userId, 'date': date});
  ...
  Future<List<Habit>> getHabits(int userId) async {
    try {
      final response = await _dio.get('/habits', queryParameters: {'userId': userId});

      if (response.statusCode == 200) {
        // Habit modelinin fromJson metodunu kullan
        return (response.data as List).map((json) => Habit.fromJson(json)).toList();
      }
      return [];
    } on DioException catch (e) {
  ...
  Future<Map<String, String>?> getRandomQuote() async {
    try {
      ...
      if (response.statusCode == 200) {
        return {'text': response.data['content'], 'author': response.data['author']};
      }
      return null;
    } on DioException catch (e) {
