...
class InspirationService {
  ...
  Future<void> _loadCachedData() async {
    ...
    if (quotesJson != null) {
      try {
        ...
        _cachedQuotes =
            decoded
                .map((item) => {'text': item['text'] as String, 'author': item['author'] as String})
                .toList();
    ...
    if (activitiesJson != null) {
      try {
        ...
        _cachedActivities = decoded.map((item) => Map<String, dynamic>.from(item)).toList();
      } on Exception catch (e) {
  ...
  void _setDefaultQuotes() {
    _cachedQuotes = [
      ...
      {'text': 'Başarı hazırlık ile fırsat buluştuğunda ortaya çıkar.', 'author': 'Zig Ziglar'},
      {'text': 'Küçük adımlar, büyük değişimlere yol açar.', 'author': 'ZenVivaTodo'},
      {'text': 'İyi alışkanlıklar, büyük sonuçlar doğurur.', 'author': 'Aristotle'},
      {'text': 'Her başarı bir hayalle başlar.', 'author': 'Walt Disney'},
    ];
  }
  ...
  void _setDefaultActivities() {
    _cachedActivities = [
      {'activity': '15 dakika meditasyon yap', 'type': 'rahatlatıcı', 'participants': 1},
      {
        'activity': '10 dakika boyunca stretching egzersizleri yap',
        'type': 'fiziksel',
        'participants': 1,
      },
      {'activity': 'Su içme hatırlatıcısı kur', 'type': 'sağlık', 'participants': 1},
      {'activity': 'Bir arkadaşına mektup yaz', 'type': 'sosyal', 'participants': 2},
      {'activity': 'Yeni bir yemek tarifi dene', 'type': 'yemek', 'participants': 1},
    ];
  }
